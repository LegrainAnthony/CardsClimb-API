...
describe('Authentication Module (e2e)', () => {
  ...
  it('should fail to retrieve the current user with a fake token', async () => {
   ...
   // Payload: {"id": 3}
   // signature : ??
    const fakeToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIzIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.cGMHEG9WFX1TiiUc1oyrJe0mOBOALWB6Jkam1VpQFSg';
  ...
  it('should validate the first card of the day', async () => {
    ...
    const { box_steps } = await boxesService.getBoxWithBoxSteps(cardToReview.box_id, userId);
    const currentBoxStep = box_steps.find((boxStep) => boxStep.id === cardToReview.box_step_id);

    const futurBoxStep = box_steps.find((boxStep) => currentBoxStep!.order + 1 === boxStep.order);
